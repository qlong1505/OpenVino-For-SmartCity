#include "Tracker.h"
#define MIN_SIZE 0.0016 //made by Long
/* ==========================================================================
...
void SingleTracker::calcAvgPos()
{
	...
	if (this->c_q.size() == full)
	{ // Start calculating the averages when the queue is full.
		for (int i = 0; i < 5; i++)
		{
...
int SingleTracker::markForDeletion()
{
	...
	const int frames = 5; // Arbitrary numbers, adjust if needed
...
int TrackingSystem::initTrackingSystem()
{
	...
	for (auto &&i : this->init_target)
	{
		...
		if ((double)i.first.area() / (double)(getFrameWidth() * getFrameHeight()) < MIN_SIZE && label == LABEL_CAR)
			continue;
...
int TrackingSystem::updateTrackingSystem(std::vector<std::pair<cv::Rect, int>> updated_results)
{
	...
	for (auto &&i : updated_results)
	{
		...
		///replace fixed value by a MIN_SIZE
		if ((double)i.first.area() / (double)(getFrameWidth() * getFrameHeight()) < MIN_SIZE && label == LABEL_CAR)
			continue;
